...
- `src/mcp/` — MCP-слой: переиспользуемые инструменты встречи (для агента инсайтов) и HTTP-эндпоинт `/mcp` — единый MCP-сервер `meeting-tasks` над данными приложения:
  ...
  **HTTP-эндпоинт `/mcp` (`meeting-tasks`):** поверх `McpController`+`McpService` регистрируются примитивы из регистраторов (сейчас — `TaskTools` из `TasksModule`): инструменты `findTask` (zod/v4: `query`, `meeting_id`, `user_id`; аннотации `title`/`readOnlyHint: true`/`idempotentHint`; query — регистронезависимая подстрока по названию) и `addTask` (zod/v4: `meeting_id`, `user_id`, `title`, `assignee?`, `source?`; `readOnlyHint: false`; создаёт задачу через `TasksService.createTask`); ресурсы `tasks://open` (статический, открытые задачи всех встреч через `TasksService.listOpenTasks`) и `task://{id}` (динамический шаблон, задача по id через `TasksService.getTaskById`; авторизация не реализована — доступен любой id); промпты `meeting_brief` (бриф встречи) и `meeting_task_review` (встреча + агрегированные задачи). Перед доступом к данным инструментов `MeetingOwner` проверяет, что встреча принадлежит `user_id`. Имя/версия сервера попадают в `serverInfo` ответа `initialize` (`MCP_SERVER_NAME`/`MCP_SERVER_VERSION`). Без `Accept: application/json, text/event-stream` — 406, без JSON `Content-Type` — 415 (валидация транспорта).
