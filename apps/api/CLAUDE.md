...
- `src/profile/` — профиль авторизованного пользователя:
  - `ProfileController` — `GET /profile` (JWT) возвращает `{ id, email, name }` текущего пользователя через `FindUserByIdQuery` (поле аватара добавится в фазе 2); без токена 401, при отсутствии пользователя — 404.
