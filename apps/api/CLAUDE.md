...
- `src/meetings/` — базовый CRUD без бизнес-логики:
  ...
  - `meetings.repository.ts` — in-memory (по образцу UsersRepository), `create(userId, ...)`, `findAll`, `findById`, `updateSummary`, `insert` (для seed автономного `mcp-server.ts`), `clear()`;
...
- `src/mcp/` — переиспользуемые MCP-инструменты встречи поверх Task/Meeting (готовы к выносу в отдельный MCP-сервер):
  ...
  - `meeting-owner.ts` — сервис владельца встречи: `MeetingOwner.findOwnedByUser(userId, meetingId)` возвращает встречу, если она существует и принадлежит userId; иначе `MeetingNotFoundError`/`MeetingNotOwnedError` (инструмент мапит в `isError: true`). Проверка выполняется до возврата данных — `meeting_id` приходит извне, и без неё любой пользователь мог бы запросить задачи чужой встречи.
  - `mcp-server.ts` — автономный MCP-сервер `meeting-tasks` с транспортом **stdio** (`@modelcontextprotocol/sdk`: `McpServer` + `StdioServerTransport`, CJS-совместим). Имя/версия задаются в конструкторе `McpServer` из SDK и попадают в `serverInfo` JSON-описания сервера (ответ `initialize`). Зависимости — `MeetingTasksMcpServerDeps { meetingsRepository, tasksService }`: весь доступ к данным задач идёт через единый сервисный слой `TasksService` (инстанцируется напрямую в `main()`, репозиторий задач наружу не торчит). Регистрирует:
    - инструменты `findTask` (zod/v4: `query`, `meeting_id`, `user_id`; аннотации `title`/`readOnlyHint: true`/`idempotentHint`; query — регистронезависимая подстрока по названию) и `addTask` (zod/v4: `meeting_id`, `user_id`, `title`, `assignee?`, `source?`; `readOnlyHint: false`; создаёт задачу через `TasksService.createTask`);
    - ресурсы `tasks://open` (статический, список открытых задач всех встреч через `TasksService.listOpenTasks`) и `task://{id}` (динамический шаблон, задача по id через `TasksService.getTaskById`; авторизация не реализована — доступен любой id);
    - промпты `meeting_brief` (бриф встречи) и `meeting_task_review` (встреча + агрегированные задачи), возвращают структурированные сообщения с подставленным текстом.
      Перед доступом к данным инструментов `MeetingOwner` проверяет, что встреча принадлежит `user_id`. `createMeetingTasksMcpServer(deps)` и регистрирующие функции (`registerFindTaskTool`, `registerAddTaskTool`, `registerOpenTasksResource`, `registerTaskByIdResource`, `registerMeetingBriefPrompt`, `registerMeetingTaskReviewPrompt`) переиспользуются в jest через in-memory транспорт (`test/mcp-server.e2e-spec.ts`, без моков). Точка входа `main()` — `node dist/mcp/mcp-server.js`; при `MCP_SEED_FILE=<json>` грузит seed-данные (встречи + задачи) в in-memory репозитории — нужно клиентскому тесту (`test/mcp-server.real.test.mjs`). В stdout пишет только MCP-протокол, логи/ошибки — в stderr.

  **ESM-зависимость:** `meeting-tool.ts` импортирует ESM-only SDK (`tool`, `createSdkMcpServer`) — работает через `require(esm)` на Node ≥ 22.12, как и `ClaudeAgentService`. В jest SDK мокается pass-through'ом (`tool`/`createSdkMcpServer` возвращают определения, чтобы тестировать хендлеры напрямую). `mcp-server.ts` использует `@modelcontextprotocol/sdk` (прямая зависимость API) — он CJS-совместим, поэтому `test/mcp-server.e2e-spec.ts` гоняет реальный протокол через `InMemoryTransport`, а `test/mcp-server.real.test.mjs` — через подпроцесс и `StdioClientTransport`.
...
- `npm run test:mcp --workspace @video-meetings/api` — клиентский тест MCP-сервера: собирает API и гоняет `node --test test/mcp-server.real.test.mjs` (подпроцесс `dist/mcp/mcp-server.js` + MCP-клиент; seed через `MCP_SEED_FILE`)
